		...
		for root, dirs, files in os.walk(url):
			for file in files:
				if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
					DP.update(0,"Fixing", "[COLOR blue]%s[/COLOR]" % file, "Please Wait")
...
def killxbmc():
	choice = DIALOG.yesno('Force Close', 'You are about to force close', 'Would you like to continue?', nolabel='No, Cancel',yeslabel='Yes, Close')
	if choice == 0: return
	elif choice == 1: pass
	myplatform = platform()
	...
	if myplatform == 'osx': # OSX
		...
		DIALOG.ok("[COLOR=red][B]WARNING !!![/COLOR][/B]", "If you\'re seeing this message it means the force close", "was unsuccessful. Please force close manually [COLOR=lime]DO NOT[/COLOR] exit cleanly via the menu.",'')
		...
		DIALOG.ok(ADDONTITLE, "Press the HOME button on your remote, go to the settings menu, then select either Apps or the Manage Installed Applications menu. Locate Kodi or variant and select Force Stop")
		...
		DIALOG.ok("[COLOR=red][B]WARNING !!![/COLOR][/B]", "If you\'re seeing this message it means the force close", "was unsuccessful. Please force close manually [COLOR=lime]DO NOT[/COLOR] exit cleanly via the menu.","Use task manager and NOT ALT F4")
	else: #ATV
		...
		DIALOG.ok("[COLOR=red][B]WARNING !!![/COLOR][/B]", "If you\'re seeing this message it means the force close", "was unsuccessful. Please force close manually [COLOR=lime]DO NOT[/COLOR] exit via the menu.","iOS detected. Press and hold both the Sleep/Wake and Home button for at least 10 seconds, until you see the Apple logo.")
