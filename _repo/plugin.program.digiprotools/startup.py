...
def checkUpdate():
	...
	if len(match) > 0:
		...
		if version > BUILDVERSION:
			yes_pressed = DIALOG.yesno(ADDONTITLE,"New version of your current build avaliable: %s v%s" % (BUILDNAME, version), "Click Build Page to install update.", yeslabel="Build Page", nolabel="Ignore for 3 days")
			if yes_pressed:
...
if not WORKING:
	...
	yes_pressed = DIALOG.yesno(ADDONTITLE,"Current build not installed by %s." % ADDONTITLE, "Select 'Build Menu' to install a build", yeslabel="Build Menu", nolabel="Ignore")
